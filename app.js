...
async function extractTextPage(page) {
  ...
  const totalLen = items.reduce((s, i) => s + i.text.length, 0);
  return { table: itemsToTable(items), totalLen };
}
...
async function convertOneFile(file) {
  ...
  for (let pageNum = 1; pageNum <= totalPages; pageNum++) {
    ...
    const { table: textTable, totalLen } = await extractTextPage(page);

    let table = textTable;
    let sourceNote = "نص";
    if (totalLen < MIN_TEXT_LEN_FOR_TEXT_PAGE) {
